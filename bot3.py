...
# --- RATE LIMITER ---
class RateLimiter:
    def __init__(self, max_calls_per_second=25):
...
"""
    ...
    markup.add(types.InlineKeyboardButton("📢 CHANNEL", url="https://t.me/cyber_404io"), types.InlineKeyboardButton("👤 OWNER", url=f"tg://user?id={ADMIN_ID}"))
...
def process_cc(cc, message, stats):
    ...
    if is_card_expired(cc):
        with stats['_counter_lock']:
            stats['checked'] += 1
        return
    
    with stats['_state_lock']:
        stats['last_cc'] = cc
...
def handle_docs(message):
    ...
    with active_checks_lock:
        if len(active_checks) >= 5:
            bot.reply_to(message, f"{get_emj('⏳')} Server busy! {len(active_checks)} users are checking. Try again in a moment.")
            return
    ...
    try:
        ...
        try:
            for idx, cc in enumerate(lino):
                # Check stop_event before submitting each CC
                if stats['stop_event']:
                    logger.info(f"User {message.chat.id}: Stop requested, breaking at {idx}/{len(lino)}")
                    break
...
if __name__ == "__main__":
    try:
        bot.delete_webhook()
        logger.info("Fast Bot is running - EXPIRED cards are SILENTLY SKIPPED")
